...
function maskPhone(value: string | null | undefined): string {
  ...
  return `${visibleStart}${"•".repeat(digits.length - 4)}${visibleEnd}`;
}
...
export default async function CallLogsPage({
  ...
  const [{ calls, configured, error }, summariesRaw, smartPbxEvents] = await Promise.all([
    configuredAuth
    ...
    useDateFilter
      ? listAgentCallEvents(agent.id, { limit: 500, startIso: ymd(start), endIso: ymd(endFilter) })
      : listAgentCallEvents(agent.id, { limit: 500 }),
  ]);
